...
def scale_and_plot(data, scaler, x_label, y_label):
    ...
    plt.xlabel(" CO2 emission(metric tons per capita) in 1990")
    plt.ylabel("Growth per year [%]")
    plt.savefig('Co2 emissionpng')
    plt.show()
    return norm
...
def plot_exponential_growth(df, popt):  # plot exponentialgrowth
    ...
    plt.savefig('Exponential Growth Model')
    plt.show()
...
def plot_with_error(df, popt, pcovar):
    ...
    plt.savefig('Logistics Model with Error Bars')
    plt.show()
